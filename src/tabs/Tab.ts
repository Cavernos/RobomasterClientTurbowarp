...
export class Tab {
    ...
    [key: string]:
        | unknown
        | ((args?: object | undefined) => Promise<Response | undefined>)
    ...
    constructor(
        ...
        for (const block of this.blocks) {
            if (block instanceof Block) {
                this[block.opcode.toString()] = async (
    ...
    getInfo(): Scratch.Info {
        this.blocks.forEach(
            (block: Scratch.Block | Scratch.Separator, index: number) => {
                if (block instanceof Block) {
        ...
        return {
            id: this.id,
            name: (language.getMessage(this.id, 'NoName') as string)
                .match(/[A-Z][a-z]+/g)!
                .join(' '),
            color1: this.color,
            color2: this.color,
